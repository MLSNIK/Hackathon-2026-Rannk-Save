...
authRouter.patch('/me', requireAuth, async (req, res, next) => {
  try {
    const { displayName, email, password, walletAddress, avatar,
            savingsWalletAddress, savingsPercent, savingsEnabled } = req.body as {
      displayName?: string;
      email?: string;
      password?: string;
      walletAddress?: string;
    ...
    if (savingsWalletAddress !== undefined) {
      const trimmed = savingsWalletAddress.trim();
      updates.savingsWalletAddress = trimmed ? normaliseWalletAddress(trimmed) : null;
    }
    if (savingsPercent !== undefined) {
      const pct = Math.round(Number(savingsPercent));
      if (Number.isNaN(pct) || pct < 0 || pct > 100) {
        res.status(400).json({ error: 'savingsPercent must be between 0 and 100' });
        return;
      }
      updates.savingsPercent = pct;
    }
    if (savingsEnabled !== undefined) updates.savingsEnabled = Boolean(savingsEnabled);
