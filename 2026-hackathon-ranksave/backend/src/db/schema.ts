...
export const users = sqliteTable('users', {
  ...
  // ─── Pension savings ────────────────────────────────────────────────────────
  // When enabled, every payment the user sends also charges an extra `savingsPercent`
  // (on top of the bill) into their own `savingsWalletAddress` — a second real
  // Interledger wallet they control. See routes/remit.ts + routes/callback.ts.
  savingsWalletAddress: text('savings_wallet_address'),     // canonical https:// URL, nullable
  savingsPercent:       integer('savings_percent'),         // whole percent, e.g. 10 = 10%
  savingsEnabled:       integer('savings_enabled', { mode: 'boolean' }),
...
export const transactions = sqliteTable('transactions', {
  ...
  //   'PAYMENT' (or null) — a normal send to a recipient
  //   'SAVINGS'           — the auto-save top-up to the user's own savings wallet,
  //                         created alongside a PAYMENT and linked via parentTransactionId
  kind:                  text('kind'),
  // For a SAVINGS row: the id of the PAYMENT it rode along with (one consent funds both).
