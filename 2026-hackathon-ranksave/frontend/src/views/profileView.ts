...
export async function renderProfileView(container: HTMLElement): Promise<void> {
    ...
    <div class="card profile-card">
      ...
      <form id="profile-form" novalidate>
        ...
        <div class="savings-settings">
          <div class="savings-settings-head">
            <h3 class="savings-settings-title">🔮 Auto-save for future you</h3>
            <label class="switch">
              <input type="checkbox" id="p-savings-enabled" ${user.savingsEnabled ? 'checked' : ''} />
              <span class="switch-slider"></span>
            </label>
          </div>
          <p class="muted">Spend like normal — we'll quietly stack a slice into your own savings wallet on top of every payment. Running low? Your payment always comes first.</p>

          <div class="field">
            <label for="p-savings-wallet">Savings wallet address</label>
            <input id="p-savings-wallet" type="text" class="input"
              placeholder="$ilp.interledger-test.dev/your-savings"
              value="${escapeHtml(user.savingsWalletAddress ? toPointer(user.savingsWalletAddress) : '')}" />
          </div>

          <div class="field">
            <label for="p-savings-percent">Save percentage</label>
            <div class="amount-wrap">
              <input id="p-savings-percent" type="number" min="0" max="100" step="1" class="input"
                placeholder="10" value="${user.savingsPercent ?? ''}" />
              <span class="amount-currency">%</span>
            </div>
            <span class="field-hint">e.g. 10 means 10% of each payment is also saved.</span>
          </div>
        </div>
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      const savingsPercentRaw = form.querySelector<HTMLInputElement>('#p-savings-percent')!.value.trim();
      ...
      const data: Record<string, unknown> = {
        ...
        // Pension savings settings
        savingsWalletAddress: form.querySelector<HTMLInputElement>('#p-savings-wallet')!.value.trim(),
        savingsEnabled:       form.querySelector<HTMLInputElement>('#p-savings-enabled')!.checked,
        savingsPercent:       savingsPercentRaw === '' ? 0 : Number(savingsPercentRaw),
      };
