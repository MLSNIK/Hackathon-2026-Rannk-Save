...
// The pension auto-save outcome that rode along with this payment.
function savingsRow(tx: Transaction): string {
  const s = tx.savings;
  if (!s) return '';
  ...
  if (s.status === 'COMPLETED') {
    return `
      <div class="summary-row summary-row-savings">
        <span class="label">🏦 Saved to pension</span>
        <span class="value">+ ${formatMoney(s.debitAmount, s.assetCode, s.assetScale)}</span>
      </div>`;
  }
  if (s.status === 'FAILED') {
    return `
      <div class="summary-row">
        <span class="label">🏦 Pension save</span>
        <span class="value muted">Skipped — payment came first</span>
      </div>`;
  }
  // PENDING / AWAITING_GRANT — still settling
  return `
    <div class="summary-row">
      <span class="label">🏦 Pension save</span>
      <span class="value muted">Processing…</span>
    </div>`;
}
