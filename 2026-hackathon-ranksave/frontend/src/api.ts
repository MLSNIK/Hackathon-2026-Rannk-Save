...
export interface QuoteResponse {
  ...
  /** Present when the user has pension auto-save on: the extra amount charged on
   *  top of the bill into their own savings wallet, funded by the same consent. */
  savings?: {
    transactionId: string;
...
export interface Transaction {
  ...
  /** The linked pension top-up's outcome, when this payment had auto-save on. */
  savings?: {
    status:      'PENDING' | 'AWAITING_GRANT' | 'COMPLETED' | 'FAILED';
    debitAmount: string | null;
    assetCode:   string;
...
export interface User {
  ...
  // Pension auto-save settings
  savingsWalletAddress?: string | null;
  savingsPercent?:       number | null;
  savingsEnabled?:       boolean | null;
}

/** GET /api/savings — the pension dashboard payload. */
export interface SavingsSummary {
  enabled:       boolean;
  percent:       number;
  walletAddress: string | null;
  totalSaved:    { value: string; assetCode: string | null; assetScale: number | null };
  contributions: Array<{
    id:                  string;
    status:              'PENDING' | 'AWAITING_GRANT' | 'COMPLETED' | 'FAILED';
    debitAmount:         string | null;
...
export const api = {
  ...
  savings: {
    get: () => get<SavingsSummary>('/api/savings', true),
  },
